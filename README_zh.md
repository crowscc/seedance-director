...
### Phase 3：素材制备（普通/专业双模式）

默认始终为**普通模式**（跳过帧图生成，直接进入分镜）。仅当你主动要求生成帧图时，才进入**专业模式**。

**普通模式**检查你的素材情况：

- **已有角色参考图？** → 跳过 Phase 3，直接进入分镜
- **没有参考图？** → 生成角色三视图（仅三视图，不生成帧图），然后进入分镜

**专业模式**（主动要求时）：为每个关键画面生成完整构图的参考帧图 — 角色（@引用）+ 背景场景 + 画面内文字元素，用于精确控制视觉效果。

多个素材通过 subagent **并行生成**。

**快速通道**：如果你同时提供了角色参考图 + 完整分镜表（含镜号/画面/台词/时长），Phase 2（分镜设计）和 Phase 4（用户确认）会被跳过，直接生成提示词。
...
多段视频**默认使用视频延长** — BGM/语音/画面衔接天然连贯，一致性最好：

| 优先级 | 策略 | 适用场景 |
|--------|------|---------|
| **默认首选** | **视频延长**（链式） | 绝大多数场景。BGM/语音/画面天然连贯 |
| 备选 | **独立生成 + 首帧衔接** | 场景完全跳转、需要单独重做某段不影响其他段 |
| 特殊 | **完全独立生成** | 蒙太奇、风格切换等极端场景 |

**链式依赖提醒**：视频延长是链式的（段1 → 延长出段2 → 延长出段3），段 N 不满意则段 N+1 及之后都要重来。建议每段生成 2-3 个版本，选定最佳后再延长下一段。
