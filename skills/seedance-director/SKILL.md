...
**提问优先级**（影响大的先问）：主题（如不明确）> 叙事结构 > 视觉风格 > 时长宽高比 > 素材 > 声音。3 轮用完后，剩余未确认的维度由系统根据已有信息自动决策，在 Phase 3/4 入口处一并告知用户。
...
**即梦生成时长固定为 15s**：即梦每次生成的视频统一为 15 秒。每个提示词对应一个 15s 片段，内含多个分镜（如镜头1: 0-3s → 镜头2: 3-7s → 镜头3: 7-12s → 镜头4: 12-15s）。最终在剪映中裁剪取每段最精彩的部分（如取 4-8s）并拼接成完整视频。
...
3. 参数设置（时长固定 15s / 宽高比 / 分辨率）
4. 生成后检查（主体清晰度/运镜流畅度/素材一致性/声音同步）

2 段额外：段1正常生成 → 段2使用「视频延长」→ 衔接检查
3 段+额外：逐段生成（每段 15s）→ 截取末帧 → 下段引用末帧+三视图+场景图 → 每段独立可重试 → 剪映裁剪+拼接

**剪映职责（仅限）**：裁剪每段到目标时长（取最精彩片段）、按分镜顺序拼接、添加转场和花字。**不在剪映做配音/BGM** — 所有音频（对白、旁白、BGM、环境音）全部在即梦提示词中生成。
